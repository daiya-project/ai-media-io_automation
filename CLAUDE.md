...
## 폴더 구조
- `data/` — 엑셀 파일을 여기에 넣으면 됨 (sample.xlsx 참고)
...
### 1. 엑셀 파일 준비
`data/sample.xlsx`를 참고하여 입력 데이터를 준비하고 `data/` 폴더에 넣는다.
...
### 2. 문서 생성 실행
```bash
python3 src/generate.py --input data/<엑셀파일>
```
...
## Commands
- 문서 생성: `python3 src/generate.py -i data/<엑셀파일>`
- 문서 생성 (PDF 제외): `python3 src/generate.py -i data/<엑셀파일> --no-pdf`
